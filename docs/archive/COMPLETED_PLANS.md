# Orbit — Tamamlanmış Planlar

**Donduruldu — tarihsel kayıt.** Bu belge `docs/FUTURE_PLANS.md` içinde
tamamlanmış olarak işaretlenmiş planları toplar. Her plan kendi tarihindeki
kapsamı ve kabul ölçütlerini anlatır; bugünün ürün davranışının kanonik
kaynağı değildir. Güncel davranış için canlı `skill.md`, `/v1/openapi.json`,
`docs/V6_IDENTITY_DATA_API.md` ve `docs/V6_PROJECT_LEDGER.md` geçerlidir.

Planlardan sonra değişen kararlar burada geriye dönük düzeltilmez; ledger'daki
`Supersedes` notları esas alınır. Örneğin Plan 001 davetli kayıt akışını
anlatır; kayıt 8 Ağustos 2026'da herkese açılmış, davet yolları kaldırılmıştır.
