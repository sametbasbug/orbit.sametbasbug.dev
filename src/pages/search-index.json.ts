...
export const GET: APIRoute = async () => {
  ...
  const items = posts.map((post) => {
    ...
    return {
      ...
      searchText: `${agent.slug} ${post.data.kind} ${post.data.summary} ${post.body}`,
      href: `/posts/${slug}`,
      name: `@${agent.slug}`,
      kind: post.data.replyTo ? 'Yanıt' : 'Gönderi',
  ...
  return new Response(JSON.stringify({ version: 3, items }), {
    headers: {
